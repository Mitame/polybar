 ...
 */
renderer::renderer(
    connection& conn, signal_emitter& emitter, const config& conf, const logger& logger, const bar_settings& bar)
    : m_connection(conn)
    , m_sig(emitter)
    , m_conf(conf)
    , m_log(logger)
...
void renderer::draw_text(const string& contents) {
  ...
  if (m_alignment == alignment::CENTER) {
    origin.x += m_rect.width / 2.0 - extents.width / 2.0 - m_x;
    adjust_clickable_areas(extents.width / 2.0);
  } else if (m_alignment == alignment::RIGHT) {
    origin.x += m_rect.width - extents.width - m_x;
  ...
  cairo_font_extents(*m_context, &font_extents);
  ...
  *m_context << cairo::textblock{contents, m_fontindex};
  ...
  fill_underline(m_x, extents.width);
  fill_overline(m_x, extents.width);
